function [subj] = feature_select_kruskalwallis(subj,data_patin,regsname,selsgroup,varargin)
...
%
% For each iteration: call the kruskalwallis on the DATA_PATIN data,
...
% to be created. By default, this will be 'kruskalwallis' if
% STATMAP_FUNCT = 'statmap_kruskalwallis' etc.
%
% NEW_MASKSTEM (optional, default = DATA_PATIN +
% 'kruskalwallisthresh'). The name of the new thresholded boolean
% mask group to be created from the kruskalwallis statmap. You'll
...
% mask that gets created from the kruskalwallis statmap. If THRESH =
% [], the thresholding doesn't get run
%
% STATMAP_FUNCT (optional, default = 'statmap_kruskalwallis'). Feed
% in a function name and this will create a function handle
% to that and use it to create the statmaps instead of
% statmap_kruskalwallis
%
...
%
% e.g. subj = feature_select_kruskalwallis( ...
...
defaults.statmap_funct = 'statmap_kruskalwallis';
defaults.statmap_arg = struct([]);
args = propval(varargin,defaults);

if isempty(args.new_map_patname)
  % get the name of the function being run, e.g. 'statmap_kruskalwallis' -> 'kruskalwallis'
...
for n=1:nIterations
  ...
  if ~isempty(args.thresh)
    ...
    % statmap pattern returned by the kruskalwallis
...
disp(' ');
disp(sprintf('Pattern statmap group ''%s'' and mask group ''%s'' created by feature_select_kruskalwallis', ...
